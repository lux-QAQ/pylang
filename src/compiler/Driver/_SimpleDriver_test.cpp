...
print("not", not (a == 10))
    ...
    std::vector<std::string> expected = { "add 13", "sub 7", "mul 30", "floordiv 3", "mod 1",
        "pow 1000", "lshift 20", "rshift 5", "and 2", "or 14", "xor 9", "neg -10", "pos 10",
        "inv -11", "not False" };
...
print("or_op", check(False, 4) or check(True, 5) or check(True, 6))
    ...
    std::vector<std::string> expected = { "chained_true True", "chained_false False", "eval 1",
        "eval 2", "and_op False", "eval 4", "eval 5", "or_op True" };
...
test_exc(3)
    ...
    run_aot_test(shared_driver(), "multi_except", code,
...
test_with_error()
    ...
    run_aot_test(shared_driver(), "try_else_finally", code,
...
print("not_in", "xyz" not in s)
    ...
    run_aot_test(shared_driver(), "string_ops", code,
...
print("list_unpack", a, b, c)
    ...
    run_aot_test(
        shared_driver(), "multi_assign", code, { "multi 10 10", "swap 2 1", "list_unpack 10 20 30" });
}
...
make_it_quack(p)
    ...
    run_aot_test(shared_driver(), "polymorphism", code, { 
        "Duck:", "Quack", 
        "Person:", "I'm quacking like a duck" 
    });
}
...
print("Order JSON:", o.to_json())
    ...
    // 假设 str() 和基本字符串拼接已实现
    run_aot_test(shared_driver(), "mixin", code, { 
        "User JSON: {user: Alice}", 
        "Order JSON: {order_id: 101}" 
    });
}
...
print("Static via instance:", t1.calc(5))
    ...
    run_aot_test(shared_driver(), "class_static_methods", code, { 
        "Count: 2", 
        "Static: 20",
        "Static via instance: 10"
    });
}
...
print("After invalid:", c.temperature)
    ...
    run_aot_test(shared_driver(), "properties", code, { 
        "Normal: 37", 
        "Too cold!",
        "After invalid: 37" 
    });
...
print("issubclass(A, B):", issubclass(A, B))
    ...
    run_aot_test(shared_driver(), "isinstance_check", code, { 
        "isinstance(b, B): True",
        "isinstance(b, A): True",
