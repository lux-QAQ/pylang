	...
	std::vector<std::string> varnames)
{
	...
	auto *result = PYLANG_ALLOC(PyCode,
		std::move(function),
		std::move(cell2arg),
		arg_count,
		std::move(cellvars),
	...
	PyString *name) const
{
	...
	if (!RuntimeContext::has_current() || !RuntimeContext::current().has_interpreter()) { abort(); }
	...
	auto *function_frame = PyFrame::create(interpreter->execution_frame(),
		...
		names(),
		nullptr);
	[[maybe_unused]] auto scoped_stack = interpreter->setup_call_stack(m_function, function_frame);
