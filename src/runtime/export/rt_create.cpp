...
py::PyObject *rt_build_dict(int32_t count, py::PyObject **keys, py::PyObject **values)
{
    ...
    for (int32_t i = 0; i < count; ++i) { dict_obj->insert(py::ensure_box(keys[i]), py::ensure_box(values[i])); }
...
{
    return rt_unwrap(py::PySlice::create(py::ensure_box(start), py::ensure_box(stop), py::ensure_box(step)));
}
